...
func testRebase(t *testing.T, when spec.G, it spec.S) {
	when("#Rebase", func() {
		...
		it.Before(func() {
			...
			fakeRunImage = fakes.NewImage("some/run", "run-image-top-layer-sha", local.IDIdentifier{ImageID: "run-image-digest"})
			fakeImageFetcher.LocalImages["some/run"] = fakeRunImage

			fakeRunImageMirror = fakes.NewImage("example.com/some/run", "mirror-top-layer-sha", local.IDIdentifier{ImageID: "mirror-digest"})
		...
		when("#Rebase", func() {
			when("run image is provided by the user", func() {
				when("the image has a label with a run image specified", func() {
					...
					it.Before(func() {
						fakeCustomRunImage = fakes.NewImage("custom/run", "custom-base-top-layer-sha", local.IDIdentifier{ImageID: "custom-base-digest"})
			...
			when("run image is NOT provided by the user", func() {
				...
				when("the image has a label with a run image mirrors specified", func() {
					...
					when("there are user provided mirrors", func() {
						...
						it.Before(func() {
							fakeImageFetcher.LocalImages["example.com/some/app"] = fakeAppImage
							fakeLocalMirror = fakes.NewImage("example.com/some/local-run", "local-mirror-top-layer-sha", local.IDIdentifier{ImageID: "local-mirror-digest"})
			...
			when("publish", func() {
				...
				it.Before(func() {
					fakeRemoteRunImage = fakes.NewImage("some/run", "remote-top-layer-sha", local.IDIdentifier{ImageID: "remote-digest"})
