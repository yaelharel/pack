...
func testPhase(t *testing.T, when spec.G, it spec.S) {
	var (
		...
		subject        *build.ConcretePhaseManager
	...
	it.Before(func() {
		...
		h.AssertNil(t, err)
		lifecycle, err = CreateFakeLifecycle(filepath.Join("testdata", "fake-app"), docker, logger, repoName)
		h.AssertNil(t, err)
		subject = build.NewConcretePhaseManager(lifecycle)
	})
	...
	when("Phase", func() {
		when("#Run", func() {
			...
			it("attaches the same layers volume to each phase", func() {
				writePhase, err := subject.New("phase", subject.WithArgs("write", "/layers/test.txt", "test-layers"))
				h.AssertNil(t, err)
				assertRunSucceeds(t, writePhase, &outBuf, &errBuf)
				h.AssertContains(t, outBuf.String(), "[phase] write test")
				readPhase, err := subject.New("phase", subject.WithArgs("read", "/layers/test.txt"))
				h.AssertNil(t, err)
			...
			it("attaches the same app volume to each phase", func() {
				writePhase, err := subject.New("phase", subject.WithArgs("write", "/workspace/test.txt", "test-app"))
				h.AssertNil(t, err)
				assertRunSucceeds(t, writePhase, &outBuf, &errBuf)
				h.AssertContains(t, outBuf.String(), "[phase] write test")

				readPhase, err := subject.New("phase", subject.WithArgs("read", "/workspace/test.txt"))
				h.AssertNil(t, err)
				assertRunSucceeds(t, readPhase, &outBuf, &errBuf)
			...
			it("copies the app into the app volume before the first phase", func() {
				readPhase, err := subject.New("phase", subject.WithArgs("read", "/workspace/fake-app-file"))
				h.AssertNil(t, err)
				assertRunSucceeds(t, readPhase, &outBuf, &errBuf)
				...
				deletePhase, err := subject.New("phase", subject.WithArgs("delete", "/workspace/fake-app-file"))
				h.AssertNil(t, err)
				assertRunSucceeds(t, deletePhase, &outBuf, &errBuf)
				h.AssertContains(t, outBuf.String(), "[phase] delete test")

				readPhase2, err := subject.New("phase", subject.WithArgs("read", "/workspace/fake-app-file"))
				h.AssertNil(t, err)
			...
			when("app is a zip", func() {
				it("preserves original mod times", func() {
					var err error
					lifecycle, err = CreateFakeLifecycle(filepath.Join("testdata", "fake-app.zip"), docker, logger, repoName)
					h.AssertNil(t, err)
					subject = build.NewConcretePhaseManager(lifecycle)
			...
			when("is posix", func() {
				...
				when("restricted directory is present", func() {
					...
					it("returns an error", func() {
						logger := ilogging.NewLogWithWriters(&outBuf, &outBuf)
						lifecycle, err = CreateFakeLifecycle(tmpFakeAppDir, docker, logger, repoName)
						h.AssertNil(t, err)
						subject = build.NewConcretePhaseManager(lifecycle)

						readPhase, err := subject.New(
							"phase",
							subject.WithArgs("read", "/workspace/fake-app-file"),
						)
						h.AssertNil(t, err)
			...
			it("sets the proxy vars in the container", func() {
				phase, err := subject.New(
					"phase",
					subject.WithArgs("proxy"),
				)
				h.AssertNil(t, err)
				assertRunSucceeds(t, phase, &outBuf, &errBuf)
			...
			when("#WithArgs", func() {
				it("runs the subject phase with args", func() {
					phase, err := subject.New("phase", subject.WithArgs("some", "args"))
					h.AssertNil(t, err)
					assertRunSucceeds(t, phase, &outBuf, &errBuf)
			...
			when("#WithDaemonAccess", func() {
				it("allows daemon access inside the container", func() {
					phase, err := subject.New(
						"phase",
						subject.WithArgs("daemon"),
						subject.WithDaemonAccess(),
					)
					h.AssertNil(t, err)
					assertRunSucceeds(t, phase, &outBuf, &errBuf)
			...
			when("#WithBinds", func() {
				...
				it("mounts volumes inside container", func() {
					phase, err := subject.New(
						"phase",
						subject.WithArgs("binds"),
						subject.WithBinds("some-volume:/mounted"),
					)
					h.AssertNil(t, err)
					assertRunSucceeds(t, phase, &outBuf, &errBuf)
			...
			when("#WithRegistryAccess", func() {
				...
				it("provides auth for registry in the container", func() {
					...
					phase, err := subject.New(
						"phase",
						subject.WithArgs("registry", registry.RepoName("packs/build:v3alpha2")),
						subject.WithRegistryAccess(registry.RepoName("packs/build:v3alpha2")),
					)
					h.AssertNil(t, err)
					assertRunSucceeds(t, phase, &outBuf, &errBuf)
			...
			when("#WithNetwork", func() {
				it("specifies a network for the container", func() {
					phase, err := subject.New(
						"phase",
						subject.WithArgs("network"),
						subject.WithNetwork("none"),
					)
					h.AssertNil(t, err)
					assertRunSucceeds(t, phase, &outBuf, &errBuf)
...
func assertAppModTimePreserved(t *testing.T, subject *build.ConcretePhaseManager, outBuf *bytes.Buffer, errBuf *bytes.Buffer) {
	readPhase, err := subject.New("phase", subject.WithArgs("read", "/workspace/fake-app-file"))
	h.AssertNil(t, err)
	assertRunSucceeds(t, readPhase, outBuf, errBuf)
